...
function MTUcalculator() {
						...
						>
							...
							<IconButton color="inherit" size="small">
								<DeleteIcon
									...
									onClick={() => {
										setSelectedProto(
											copySelectedProto.filter((a: any) => a.id !== list[i].id),
										);
									}}
								/>
							</IconButton>
	...
	const [customProtoName, setCustomProtoName] =
		React.useState("任意のプロトコル");
							...
							<Grid container>
								...
								<Grid item xs={3}>
									<Input
										...
										label="bytes"
										onChange={(event) =>
											setCustomProtoSize(Number(event.target.value))
										}
									/>
								</Grid>
								<Grid item xs={1}>
					...
					<Card>
						<CardBody>
							<Input
								...
								onChange={(event) => setTextInput(Number(event.target.value))}
							/>
							<p>
								Header size: {calculateSize("header size", 0, selectedProto)}
							</p>
							<p>
								MTU/MSS: {calculateSize("mtu/mss", textInput, selectedProto)}
							</p>
							<p>追加したプロトコル</p>
