...
const OoklaServerList = () => {
	...
	useEffect(() => {
		const fetchData = async () => {
			const res = await axios(
				"https://nw-tools-backend.suyama.ne.jp/api/serverlist" +
					"?nocache=" +
					new Date().getTime(),
			);
	...
	return (
		<div>
			<p>
				speedtest.netのサーバリストです。
				speedtest-cliのサーバID指定の際にお使い下さい
			</p>
			...
			<Skeleton isLoaded={loaded}>
				...
				>
					<TableHeader>
						...
						<TableColumn style={{ minWidth: "117px" }}>
							ロケーション
						</TableColumn>
						...
						<TableColumn style={{ minWidth: "300px" }}>
							IPv4 Addr(RDNS)
						</TableColumn>
						<TableColumn style={{ minWidth: "250px" }}>
							IPv6 Addr(RDNS)
						</TableColumn>
							...
							<TableRow key={item.id}>
								...
								<TableCell align="left">
									<IconButton
										color="inherit"
										size="small"
										href={"https://www.speedtest.net/server/" + item.id}
									>
										<SpeedIcon fontSize="small" />
									</IconButton>
								</TableCell>
								<TableCell align="left">{item.host}</TableCell>
								<TableCell align="left">{item.ipv4}</TableCell>
								<TableCell align="left">{item.ipv6}</TableCell>
								<TableCell align="left">
									{item.ipv4_asn === "無し" || item.ipv4_asn === "API-error" ? (
										...
										<Link
											href={
												"https://bgp.he.net/" + item.ipv4_asn.split(" ", 1)[0]
											}
										>
											{item.ipv4_asn}
										</Link>
								...
								<TableCell align="left">
									{item.ipv6_asn === "無し" || item.ipv6_asn === "API-error" ? (
										...
										<Link
											href={
												"https://bgp.he.net/" + item.ipv6_asn.split(" ", 1)[0]
											}
										>
											{item.ipv6_asn}
										</Link>
