...
const ipInfo = async (ip: string) => {
	const url =
		process.env.IPINFOIO_API_URL +
		ip +
		"?token=" +
		process.env.IPINFOIO_API_TOKEN;
	if (ip !== "error") {
		try {
			const response = await axios.get(url);
	...
	for (let i = 0; i < json.length + 1; i++) {
		if (i < json.length) {
			...
			let push = {
				...
				website: await getUrl(
					"https://www.speedtest.net/api/js/perform-redirect?server_id=" +
						json[i].id,
				),
				host: fqdn + ":8080", //url
