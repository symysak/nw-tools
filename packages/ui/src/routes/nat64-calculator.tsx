...
export const Route = createFileRoute("/nat64-calculator")({ component: Nat64Calculator })
...
function ipv4ToIpv6(ipv4: string, prefix: string): string {
  ...
  const base = prefix.endsWith("::") ? prefix + suffix : prefix.replace(/:$/, "") + ":" + suffix
  return base
}
...
function Nat64Calculator() {
  ...
  return (
    <div>
      ...
      <p className="text-sm text-muted-foreground mb-4">NAT64のアドレスの変換ができます。</p>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            <div className="space-y-1">
              ...
              <Input value={ipv4} onChange={(e) => setIpv4(e.target.value)} placeholder="例: 192.0.2.1" />
            </div>
            <div className="space-y-1">
              <Label>NAT64プレフィックス</Label>
              <Input value={prefix} onChange={(e) => setPrefix(e.target.value)} />
            </div>
            <Button onClick={() => setIpv6Result(ipv4ToIpv6(ipv4, prefix))}>変換</Button>
            {ipv6Result && (
              <div className="flex items-center gap-1 mt-2">
                <span className="text-sm font-mono bg-muted px-2 py-1 rounded flex-1 break-all">{ipv6Result}</span>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            <div className="space-y-1">
              ...
              <Input value={ipv6} onChange={(e) => setIpv6(e.target.value)} placeholder="例: 64:ff9b::c000:201" />
            </div>
            <Button
              onClick={() => {
            ...
            {ipv4Result && (
              <div className="space-y-2 mt-2">
                <div className="flex items-center gap-1">
                  <span className="text-sm text-muted-foreground w-28">IPv4アドレス:</span>
                  <span className="text-sm font-mono bg-muted px-2 py-1 rounded flex-1">{ipv4Result}</span>
                  <CopyButton text={ipv4Result} />
                </div>
                <div className="flex items-center gap-1">
                  <span className="text-sm text-muted-foreground w-28">プレフィックス:</span>
                  <span className="text-sm font-mono bg-muted px-2 py-1 rounded flex-1">{prefixResult}</span>
